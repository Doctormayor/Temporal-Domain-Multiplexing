...
for ii = 1:imageFrames
    currentImage = imread('mfRFP-mfRFP-A002.tif', ii);
...
%             composition_image6(ii,jj) = 0;
            ...
            [predicted_vals1, fval1] = fminsearch(@(coeffs) signalseparation6g(mfRFP,mfRFPA,squeeze(stack(ii,jj,:)),coeffs),[1 1]/2,optimset('MaxFunEvals', 10000) );
            [predicted_vals2, fval2] = fminsearch(@(coeffs) signalseparation6g(mfRFP,mfRFPA,squeeze(stack(ii,jj,:)),coeffs),[1 0]/2,optimset('MaxFunEvals', 10000) );
            [predicted_vals3, fval3] = fminsearch(@(coeffs) signalseparation6g(mfRFP,mfRFPA,squeeze(stack(ii,jj,:)),coeffs),[0 1]/2,optimset('MaxFunEvals', 10000) );
...
%             composition_image5(ii,jj) = predicted_vals(5);
	    ...
	    % channels from 1, so that all channels add up to 1.
            composition_image2(ii,jj) = 1-predicted_vals(1);
        end
    end
...
mfRFP_ch = composition_image1.*stack(:,:,1); % rsGreenF-E 
mfRFPA = composition_image2.*stack(:,:,1); % rsEGFP2-E
...
imwrite(uint16(mfRFP_ch), 'mfRFP_ch-code1.tif'); % rsGreenF-E
imwrite(uint16(mfRFPA), 'mfRFPA_ch-code1.tif'); % rsEGFP2-E
